...
export default function Button({
  ...
  const baseClasses =
    'btn-enhanced font-semibold rounded focus:outline-none focus:ring-2 focus:ring-gold disabled:opacity-60 disabled:cursor-not-allowed transition-all duration-200';

  // Variant-specific styling with golden glow effects using design tokens
  const variantClasses = {
    primary:
      'bg-gold text-black hover:bg-gold/90 hover:scale-[1.02] btn-primary shadow-sm hover:shadow-gold',
    secondary:
      'bg-white/10 text-white border border-white/20 hover:bg-white/20 hover:border-gold btn-secondary',
    outline:
      'border border-gold text-gold hover:bg-gold hover:text-black btn-outline hover:shadow-gold',
  };
