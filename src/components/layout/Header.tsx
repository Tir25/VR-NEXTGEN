...
function Dropdown({ page, isActive, onClose }: DropdownProps) {
  ...
  return (
    <div
      className={`absolute top-full left-0 mt-2 w-72 bg-black/95 backdrop-blur-md border border-white/10 rounded-xl shadow-2xl transition-all duration-300 ease-out ${
...
function DesktopNavItem({ page }: NavItemProps) {
  ...
  return (
    <div className='relative' ref={dropdownRef}>
      ...
      {/* Dropdown Menu */}
      <Dropdown page={page} isActive={isDropdownOpen} onClose={handleDropdownClose} />
    </div>
  );
}
        ...
        <div className='flex items-center justify-between'>
          ...
          <div className='hidden lg:flex items-center space-x-1'>
            {Object.entries(navigationData).map(([pageKey, page]) => (
              <DesktopNavItem key={pageKey} pageKey={pageKey} page={page} />
            ))}
          </div>
